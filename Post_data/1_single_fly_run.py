...
def video2tb(fly_id, Video_tb):
    ...
    Nst_fly = [[fly for fly in Fly_dic[str(Frame_start)].keys() if fly != fly_id]
      [np.array([math.dist(np.array(Fly_dic[frame][fly_id]["body"][:2]) * _pixel_frame ,
       np.array(Fly_dic[frame][fly]["body"][:2]) *_pixel_frame)
    ...
    Body_len_agl_TB = pd.DataFrame(Body_len_agl, columns=['Frame', "length", "B_angle", "X", "Y"])
...
# The first frame
Frame_start = max([int(i.replace("_.json","").split("_")[-1]) for i in os.listdir("csv") if ".json" in i])
Frame_end = 15000
# Arguments for moving states
## Combine the behaviors
Len_plate = [712,  712, 712, 712, 813, 813,
 813, 813, 926, 924, 925, 926, 920, 915,
 931, 931, 931, 931, 957, 959, 959, 933, 933, 942]

# store all resuDataFramere
All_videos = pd.DataFrame()

for i_index in range(len(Video_list)):
    video_id = Video_list[i_index]
    ...
    Video_tb.to_csv("Video_post/" + video_id +".csv")
